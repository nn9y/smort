...
from SMTMR.src.translate.smtlibv2.theory.util import *
...
class Fun:
    ...
    def match_indexed_type(self, op, input_list):
        ...
        """
        if not self.name.same_indexed_type(op):
            return False
    ...
    def get_indexed_instance(self, op_indices, input_indices_list):
        if isfunction(self.constraints):
            if not self.constraints(op_indices, input_indices_list):
                return None
        op_instance = self.name.get_indexed_instance(op_indices)
        input_instances = []
