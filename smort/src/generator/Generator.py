...
class Generator:
    def __init__(self, formulas, mr, args):
        ...
        self.templates = [self.mr.subst_templates[i] for i in self.valid_index_list] 
        # substituions
        self.substs_list = [] 
    ...
    def metamorphose(self, formulas, decls, defs, asserts):
        if self.args.incremental:
            ...
            k = random.randrange(len(self.substs_list))
            while len(self.substs_list[k][0]) == 0:
                self.substs_list.pop(k)
                k = random.randrange(len(self.substs_list))
            template = self.templates[k]
            i = random.randrange(len(self.substs_list[k][0]))
            term_tups, var_name_maps = self.substs_list[k]
            ...
            self.substs_list[k][0].pop(i)
            self.substs_list[k][1].pop(i)
            if len(self.substs_list[k][0]) == 0:
                self.substs_list.pop(k)
            if len(self.substs_list) == 0:
                self.end_increment = True
        else:
            for k, tups_list in enumerate(self.substs_list):
                template = self.templates[k]
                term_tups, var_name_maps = tups_list
    ...
    def generate(self):
        ...
        if (
            ...
            templates = self.templates if self.args.multiple_templates else [self.templates[self.template_index]]
