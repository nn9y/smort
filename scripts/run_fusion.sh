...
do  
    python3 $smort $sat_mr_file "${solvers_string}" -mt -ms --sat-seeds "${SMORT_HOME}seeds/${theory}/sat"
    python3 $smort $unsat_mr_file "${solvers_string}" -mt -ms --unsat-seeds "${SMORT_HOME}seeds/${theory}/unsat"
done
